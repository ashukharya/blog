...
app.post("/",function(req,res){
    Post.find({},function(err,posts){
        if(!err){
            ...
            let x=posts[posts.length-1].title;
            Post.deleteOne({ title : x },function(err){
                if(!err){
                    console.log("yeah");
                }
            });
...
app.get("/posts/:postId",function(req,res){
        ...
        Post.findOne({_id: requestedPostId}, function(err, post){
            res.render("posts", {title: post.title,content: post.content,date:post.date});
          });
});
...
//     const day=date.getAll();
//         const requestedtitle=_.lowerCase(req.params.postTitle);
//         posts.forEach(function(post){
//             const postT=_.lowerCase(post.title);
//                 if(requestedtitle===postT){
